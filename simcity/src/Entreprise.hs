...
recruterEmploye (Entreprise eid  batiments employes capital) cid p =
    let ne = Entreprise eid  batiments ((cid,p):employes) capital in
    if invariantEntreprise ne then  Just ne
    else Nothing


virerEmploye :: Entreprise -> CitId -> Maybe Entreprise
virerEmploye (Entreprise eid  batiments employes capital) cid =
    let ne =Entreprise eid  batiments (filter (\(cid',_) -> cid /= cid') employes) capital in
    if invariantEntreprise ne then Just ne
                         else   Nothing


changerPosteEmployer :: Entreprise -> CitId -> Poste -> Maybe Entreprise
changerPosteEmployer (Entreprise eid  batiments employes capital) cid p =
    let ne = Entreprise eid  batiments (map (\(cid',p') -> if cid == cid' then  (cid,p) else (cid',p')) employes) capital in
        if invariantEntreprise ne then Just ne
                            else Nothing

augmenterCapital :: Entreprise -> Float -> Maybe Entreprise
augmenterCapital (Entreprise eid  batiments employes capital) montant =
     let ne = Entreprise eid  batiments employes (capital + montant) in
        if invariantEntreprise ne then Just ne
                           else  Nothing

diminuerCapital :: Entreprise -> Float -> Maybe Entreprise
diminuerCapital e@(Entreprise eid  batiments employes capital) montant =
...
filtrerBatiment (Entreprise eid  batiments employes capital) bid =
        Entreprise eid  (filter (bid /=) batiments) employes capital
...
entreprisePasEnFaillite (Entreprise eid  batiments employes capital) = (capital > 0) || error "Votre entreprise est en faillite"

entrepriseAyantDesEmployes :: Entreprise -> Bool
entrepriseAyantDesEmployes (Entreprise _  _ employes _) = not (null employes) || error "Votre entreprise n'a pas d'employés"

entepriseAyantDesBatiments :: Entreprise -> Bool
entepriseAyantDesBatiments (Entreprise _  batiments _ _) = not (null batiments) || error "Votre entreprise n'a pas de batiments"

entrepriseAyantUnSeulCEO :: Entreprise -> Bool
entrepriseAyantUnSeulCEO (Entreprise _  _ employes _) = length (filter (\(_,p) -> p == CEO) employes) == 1 ||
